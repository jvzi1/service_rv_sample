...
if __name__ == '__main__':
    # 修改csv地址
    csv_dir = r"D:\data\rhea\rv_laneline\RV车道线点属性"
    for csv_path in os.listdir(csv_dir):
        ...
        with open(os.path.join(csv_dir,csv_path),"r", encoding="utf-8") as f:
            count = 0
            data = csv.reader(f)
            for i in data:
                if "id" in i[0]:
                ...
                summary_info["all"] += int(i[8])
                summary_info["point_array"] += int(i[8])
                data_list.append(i)
